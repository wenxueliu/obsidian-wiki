...
Always-on via `.kiro/steering/*.md` with `inclusion: always`. `python3 setup.py` symlinks `.skills/*` into both `.kiro/skills/` and `~/.kiro/skills/`. Invoke with `/wiki-folder-ingest`, `/wiki-query`, etc.
</details>
