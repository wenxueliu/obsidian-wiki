...
此 skill 直接执行下方从 `workflows/wiki-status.yaml` 同步的完整契约。内嵌 YAML 是实际指令，不是摘要或外部参考；按 `steps`、输入输出、检查、跳转、失败上限和人工审批要求逐项执行。
...
steps:
  - id: resolve_context
    desc: 复用共享子 workflow 解析 status 来源与 vault 上下文
    workflow: wiki-context
    input: status invocation 与当前 CWD
    ...
    inputs:
      ...
      setup_mode: "false"
    on_pass: resolve_and_inventory
    on_fail: resolve_context
    max_fail_count: 3

  - id: resolve_and_inventory
    desc: 解析 vault、来源配置与当前 manifest
    do: |
    ...
    input: wiki 状态、delta、dashboard、health 或 insights 请求（vault 由 wiki-context 交互式确认）
    output: status-context.md + source-inventory.json
    check_voting:
      - check: 复核 config precedence、canonical paths、所有标准 source inventories 与 manifest 外路径，fresh/archived coverage 判定准确
      - check: 抽查 page/source counts、scope exclusions 和 mode，确认只用了廉价 metadata/frontmatter 且没有任何写入
    on_pass: compute_delta
    on_fail: resolve_and_inventory
    max_fail_count: 3

  - id: compute_delta
    desc: 分类来源 delta 并计算 Wiki 概览与 token footprint
    do: |
    ...
    input: status-context.md + source-inventory.json + manifest/page frontmatter
    output: delta.json + wiki-status-report.md（概览、delta、token footprint）
    check_voting:
      - check: 重新抽样 hash/mtime/manifest 对比，确认 new/modified/touched/unchanged/deleted 互斥且 canonical keys 无重复
      - check: 重算 pages/categories/visibility/staging/source/project counts，fresh manifest 与 archive coverage 没有误报
      - check: 重算各 tier、full/index/typical token estimate 与 threshold guard，确认报告只写 artifacts
    on_pass: rank_actions
    on_fail: compute_delta
    max_fail_count: 4

  - id: rank_actions
    desc: 按固定优先级生成 What to Do Next
    do: |
    ...
    input: delta.json + index/log/hot + live link/frontmatter metadata
    output: next-actions.json + 完整 standard wiki-status-report.md
    check_voting:
      - check: 独立重算 raw/stale/orphan/synthesis/source/lint/staging signals，阈值、排除项与 counts 准确
      - check: 核对 priority 0-6、最多 6 项、overflow/empty state 和推荐 workflow，未把 touched 算作 ingest delta
      - check: 确认 standard report 全程只读，未修改 vault/log/hot/manifest/QMD
    on_pass: insights
    on_fail: rank_actions
    max_fail_count: 3

  - id: insights
    desc: 条件生成 Wiki 图谱 insights 与 snapshot
    do: |
    ...
    input: status report + graph metadata + optional previous _insights.md
    output: insights-report.md + insights-data.json；适用时重写 _insights.md
    check_voting:
      - check: 重跑 graph analyzer/fallback，核对 anchors/bridges/communities/dead ends/isolates/surprising 与 stats，skip guards 准确
      - check: 重算 cohesion、graph delta、tier thresholds/questions priority，确认 snapshot JSON 完整可供下次 diff
      - check: standard/skip 模式 vault 零变化；active insights 只改 _insights.md，未改知识页/manifest/index/hot
    on_pass: finalize_status
    on_fail: insights
    max_fail_count: 4

  - id: finalize_status
    desc: 交付状态报告并仅为 insights 记录可选写入
    do: |
    ...
    input: wiki-status-report.md + insights reports + log/QMD config
    output: wiki-status-completion.md + 条件 STATUS_INSIGHTS/QMD refresh
    check_voting:
      - check: 对照最终 source/vault metadata 抽查 status report、delta、token footprint 与 next actions，无过期或虚构 counts
      - check: standard/skip 保持完全只读；active insights 的 log run_id 恰好一次且只有 _insights.md/log.md 为 vault diff
      - check: QMD 仅在 insights write 后执行，guard/update/embed/verify 与 completion 状态准确
    on_pass: done
    on_fail: finalize_status
    max_fail_count: 3
````
