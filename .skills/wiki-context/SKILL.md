...
steps:
  ...
  - id: resolve_context
    ...
    do: |
      ...
      console script 不在 PATH 时使用等价的 `python3 -m obsidian_wiki wiki-context-resolve ...`。config mode 严格按 `@name` 指定 config、从 source CWD 向上找第一个含 `OBSIDIAN_VAULT_PATH` 的 `.env`、再到全局 `~/.obsidian-wiki/config` 的顺序解析；都缺失时要求运行 wiki-setup。resolver 将 effective config、owner/Writing Profile metadata、requested optional metadata、active Knowledge Pack 的 Knowledge Profile/Layout 状态、text chunking 配置及 retrieval order 写入声明的 artifacts。
    ...
    check: 解析 artifacts，核对 canonical vault/config/mode、requested keys 与 chunk options；请求 layout 时确认 marker matched 且 Knowledge Profile/routing/hash 冻结一致；skipped 不带 config/vault；确认除 artifacts 外零写入
    on_pass: done
    on_fail: collect_vault
    max_fail_count: 3
````
