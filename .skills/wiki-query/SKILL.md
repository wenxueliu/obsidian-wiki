...
此 skill 是 `workflows/wiki-query.yaml` 的 Agent Skill 格式投影；workflow 是行为事实来源，本文件把其字段渲染为可直接执行的 Markdown 指令。
...
进入本 skill 时先绑定本次 invocation 的 `run_id` 与绝对 `artifacts_dir`。若父 workflow 已传入这两个值，原样继承；否则运行 `obsidian-wiki artifacts-create --workflow wiki-query`，解析其 JSON 输出后绑定。
...
### 1. 复用共享子 workflow 解析只读查询上下文 (`resolve_context`)
...
- 验收通过：转到 `resolve_and_classify`。

- 验收失败：返回 `resolve_context`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 2. 解析 vault、visibility/mode 并执行 GraphRAG pre-pass (`resolve_and_classify`)
...
wiki-context.json + 用户对 compiled wiki 的问题（可含 quick/public-only/path 等限定）

#### 产出

query-context.md + graph-prepass.json

#### 验收

1. 独立复核 interactive vault、config defaults/overrides precedence、Knowledge Profile/hash/scope verdict、hot/index、query type、index-only/filtered triggers、blocked tags 与 QMD settings

2. 确认 GraphRAG 在 page-body reads 前执行，输出字段和 fallback 准确；filtered candidates 未泄露 blocked pages

3. 审计只读性和 prompt-injection 边界，用户夹带写入请求仅被路由建议，vault/log/QMD 均未改变

#### 流程控制

- 验收通过：转到 `rank_candidates`。

- 验收失败：返回 `resolve_and_classify`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 3. 用 frontmatter/index 与可选 QMD 形成最小候选集 (`rank_candidates`)
...
query-context.md + graph prepass + index/frontmatter + 可选 QMD results

#### 产出

candidate-evidence.json + retrieval-plan.md

#### 验收

1. 从 index/frontmatter 重算排名、top 5-10 与 tier tie-break，filtered/index-only 约束正确且 blocked/raw paths 未进入 compiled evidence

2. 审计 QMD guard/transport/search-mode/lex-vec syntax、fallback 与 papers separation，_raw defensive filter 生效

3. 确认只读且成本最小：index-only bodies_read为空，normal next set仅 should_read/highest candidates，没有 broad/full vault read

#### 流程控制

- 验收通过：转到 `retrieve_evidence`。

- 验收失败：返回 `rank_candidates`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 4. 按需执行 section/full read 或 bounded typed graph traversal (`retrieve_evidence`)
...
retrieval-plan.md + candidate evidence + allowed wiki/QMD pages

#### 产出

evidence-pack.md + retrieval-trace.json

#### 验收

1. 按 trace 重放 retrieval escalation，确认 summary→section→最多3 full pages→一跳/broad fallback 顺序、bodies/read counts和escalated标志真实

2. 对 relationship/path 重算 typed adjacency/BFS、direction/reverse/untyped、depth/frontier/alternate与endpoint resolution，路径/无路径结论正确

3. 审计每条 evidence citation/locator、compiled-vs-raw 分层、filtered visibility；index-only 无正文读取，任何模式均无 vault 修改

#### 流程控制

- 验收通过：转到 `synthesize_answer`。

- 验收失败：返回 `retrieve_evidence`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 5. 生成带 wikilink、trust annotation、gap 与 source path 的答案 (`synthesize_answer`)
...
query-context.md + evidence-pack.md + retrieval-trace.json + manifest project metadata

#### 产出

query-answer.md + answer-validation.md

#### 验收

1. 逐 claim 对照 evidence/locator，wikilinks、retrieval-step说明、contradictions/gaps与raw/compiled边界准确，无 unsupported synthesis

2. 重算每页 lifecycle/staleness/supersession annotation，project source_cwd/fallback及相关文件真实；未执行任何提议的修改

3. 检查 index-only免责声明、multi-hop chain、filtered无泄露和标准answer sections；除 artifacts 外 vault仍未修改

#### 流程控制

- 验收通过：转到 `log_query`。

- 验收失败：返回 `synthesize_answer`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 6. 仅追加一条 QUERY 日志并交付答案 (`log_query`)
...
已验证 query-answer.md + query-context/retrieval trace + log.md

#### 产出

唯一 QUERY log + query-completion.md + 最终答案

#### 验收

1. 复核 log 中 query_id 恰好一次、query安全转义、result_pages/mode/filtered/escalated与trace一致

2. 文件系统审计确认除log.md和artifacts外零写入，QMD index/source project/pages/index/hot/manifest未变化

3. 最终呈现与query-answer一致、citations/gaps/source code/route完整，filtered/index-only/path承诺均未被交付层破坏
