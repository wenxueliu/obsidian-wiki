...
steps:
  ...
  - id: resolve_project
    ...
    do: |
      ...
      1. 使用 context 的 canonical vault、writing profile、link format、owner rules、taxonomy、manifest/index、active layout 与 QMD；不得重新解释配置。
      ...
      4. 从 canonical directory basename 派生 clean project name，并绑定 status=matched 的 active layout 及 declared routes。记录 source CWD、git root/branch/HEAD/dirty state、语言/framework、关键入口、owner schema、canonical tags 与 link format。
    ...
    check_voting:
      - check: 独立复核 interactive vault 与 config defaults/overrides precedence、canonical source/vault paths、project name、active layout、owner schema/writing profile/tag taxonomy/link format，确认没有路径混淆
  ...
  - id: plan_distillation
    ...
    do: |
      ...
      1. 只保留三个月后仍需重新推导的知识：架构决策及理由、关键 mental model/abstractions、依赖 wiring、trade-offs、非显然经验与可复用 pattern。代码可直接回答的事实、boilerplate、routine fixes 不进 wiki。
      2. 先用 index/title/aliases/tags/summary 做 cheap canonical target pass，只打开高相关现有页面。Aggressively merge，禁止重复概念页。
      3. 根据 page-contract 的 layout-specific `routing.prompt` 选择 declared page type：项目知识使用对应 `project_*` 类型，项目入口使用 `project_overview`，通用知识使用对应 global 类型。每个 target 调用 `resolve_wiki_route.py` 从 `routing.rules` 生成，禁止硬编码 default layout 目录。
