...
此 skill 是 `workflows/wiki-lint.yaml` 的 Agent Skill 格式投影；workflow 是行为事实来源，本文件把其字段渲染为可直接执行的 Markdown 指令。
...
### 1. 复用共享子 workflow 解析 lint 配置与 owner metadata (`resolve_context`)
...
- 验收通过：转到 `build_lint_context`。

- 验收失败：返回 `resolve_context`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 2. 解析 vault、owner schema 与审计边界 (`build_lint_context`)

#### 执行

使用 wiki-context.json 建立本次审计的权威上下文。

1. 使用 context 的 canonical vault、retrieval order、owner schema、provenance、confidence/lifecycle、typed relationship allowlists 与 status=matched 的 active Knowledge Pack；不得重新解释配置。layout 不 matched 时停止审计并要求 repair/migration。
...
5. 读取 index.md 与 log.md，并从 active Knowledge Pack routing.content_roots 建立页面 inventory。所有检查排除 routing.skip_dirs/system_dirs/system_paths；staging role 不算 live knowledge page。
6. 先在内存中形成唯一 canonical lint context result；`lint-context.json` 完整承载 canonical vault path、配置来源、effective schema、active Knowledge Pack name/version/hashes/完整 routing、排除项、页面数量、index/log 状态、审计时间与稳定 audit_id，Markdown 只从同一 result 渲染、不得独立推导或补充事实，并在同一批 artifact 写入中提交二者。此步骤不得修改 vault。

#### 输入

用户任务（vault 由 wiki-context 交互式确认；默认等价于 wiki-lint --check）

#### 产出

lint-context.md + lint-context.json（vault、effective schema、active routing、scope、inventory、audit_id）
...
- 验收通过：转到 `structural_audit`。

- 验收失败：返回 `build_lint_context`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 3. 执行确定性的结构、链接、schema 与 trust 检查 (`structural_audit`)
...
lint-context.md + vault 的 index/frontmatter/link/source metadata

#### 产出

structural-findings.json + structural-audit.md

#### 验收

1. 复跑 inventory、orphan、broken link、frontmatter、summary、index、tag cluster、visibility、misc promotion 与 relationship checks，核对 findings 和计数，没有漏扫或误扫排除目录

2. 复跑 strict trust-check 并核对 JSON schema block、退出码、ledger errors/mismatches/stale/unreviewed、lifecycle/base_confidence/supersession；确认没有自动重算 confidence 或运行 trust-record

3. 审计只读性、locator 与 severity：除 artifacts 外 vault 无变化，soft summary/stale warnings未错误升级，所有 finding 都有可复现 evidence

#### 流程控制

- 验收通过：转到 `semantic_audit`。

- 验收失败：返回 `structural_audit`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 4. 聚焦相关页面执行矛盾、provenance 与 synthesis gap 审计 (`semantic_audit`)
...
4. 从 active Knowledge Pack routing 声明的 concept/entity 内容根中选择 10-15 个高频链接，两两计算共同出现页面；仅报告 co-occurrence>=3 且对应 synthesis route 下不存在页面的 gap，不假定 default Pack 目录名。
...
lint-context.md + structural findings + 相关页面的局部正文

#### 产出

semantic-findings.json + semantic-audit.md

#### 验收

1. 独立抽样复核 contradictions，确认双方原文/locator 存在，已承认矛盾与未承认矛盾区分准确，没有把措辞差异伪装为事实冲突

2. 重算 provenance 分母、marker 数、hub 排名和四个阈值，核对 drift 与 severity；未带 provenance/marker 的页面确实跳过

3. 重算 synthesis pair 共现和已有 synthesis coverage，确认候选集 10-15 个且每个报告 gap 达到 >=3；确认仅按需读取相关正文且 vault 未修改

#### 流程控制

- 验收通过：转到 `render_report`。

- 验收失败：返回 `semantic_audit`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 5. 汇总完整 Wiki Health Report (`render_report`)
...
lint-context.md + structural/semantic findings

#### 产出

wiki-health-report.md + lint-summary.json

#### 验收

1. 从两个 findings JSON 重算每节和总计，确认报告无遗漏、无重复、severity 与 soft warning 口径一致，所有路径和 locator 可定位

2. 对照 wiki-lint Output Format、effective schema 与 trust-check 结果，确认 13 类检查均有呈现，修复建议没有越过 human-only lifecycle/confidence 边界

3. 确认 report/summary 只写 artifacts，未修改 vault，未运行 QMD；报告清楚区分事实 findings、语义判断与建议

#### 流程控制

- 验收通过：转到 `record_lint`。

- 验收失败：返回 `render_report`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 6. 幂等记录 LINT 操作并完成审计 (`record_lint`)
...
已验证的 wiki-health-report.md + lint-summary.json + vault/log.md

#### 产出

一条幂等 LINT log 记录 + lint-completion.md
