...
此 skill 是 `workflows/cross-linker.yaml` 的 Agent Skill 格式投影；workflow 是行为事实来源，本文件把其字段渲染为可直接执行的 Markdown 指令。
...
进入本 skill 时先绑定本次 invocation 的 `run_id` 与绝对 `artifacts_dir`。若父 workflow 已传入这两个值，原样继承；否则运行 `obsidian-wiki artifacts-create --workflow cross-linker`，解析其 JSON 输出后绑定。
...
### 1. 复用共享子 workflow 解析 linking 上下文 (`resolve_context`)
...
linking invocation、run_condition 与当前 CWD
...
- 验收通过：转到 `build_registry`。

- 验收失败：返回 `resolve_context`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 2. 解析 vault 并建立低成本页面词汇表 (`build_registry`)
...
3. 只扫描 active Knowledge Pack routing.content_roots 下的 live `.md`，排除 routing.skip_dirs/system_dirs/system_paths；只 grep frontmatter/title/aliases/tags/category/summary，不盲读全 vault。
...
cross-link、连接 orphan 或补齐 wikilink 请求（vault 由 wiki-context 交互式确认）

#### 产出

page-registry.json + linking-context.md

#### 验收

1. 对照 index/frontmatter 抽查 registry 的 path/title/alias/tags/category/summary 和重名消歧准确

2. 确认 conditional skip 有明确 evidence 且没有扫描 vault；执行模式的 config/link format、live scope 与 exclusions 正确，未全量读取不相关正文且 vault 无变化

#### 流程控制

- 验收通过：转到 `discover_links`。

- 验收失败：返回 `build_registry`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 3. 按需读取候选页面并评分缺失链接 (`discover_links`)
...
page-registry.json + 相关页面正文

#### 产出

link-plan.json + cross-link-plan.md

#### 验收

1. skip 模式的计划为空且未读取页面；执行模式重算候选分数和 confidence，确认只含 >=3，exact/semantic/cross-category/hub signals 有正文或 registry 证据

2. 检查 self/common/duplicate/code/frontmatter/ambiguous 均排除，inline locator 自然且每个 target 只链接首次合理 mention

3. 对 typed relationships 逐项复核标准 type、方向、语境与 target；不能精确分类的确为 null，vault 尚未写入

#### 流程控制

- 验收通过：转到 `snapshot_and_apply`。

- 验收失败：返回 `discover_links`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 4. 建立可回退快照并原子应用链接计划 (`snapshot_and_apply`)
...
link-plan.json + 最新 live pages + Git 状态

#### 产出

已验证的 links/relationships + cross-link-apply-report.md

#### 验收

1. skip 模式没有 snapshot/vault diff；执行模式复核 standalone Git root 判定与 snapshot 行为，dirty standalone vault 在写前有有效 commit，clean/non-standalone 分支处理正确

2. 将实际 diff 与 plan 逐项对账，检查 link format/relative path、inline/Related placement、首次 mention、并发 hash 与无额外改动

3. 验证所有 targets 存在、relationship type/direction 合法、nested/flat-key/inline @type 三种 projection 一致，且无 duplicate/self/code/frontmatter links，tracking/QMD 尚未改变

#### 流程控制

- 验收通过：转到 `update_affinity`。

- 验收失败：返回 `snapshot_and_apply`。

- 最多连续失败 `4` 次；达到上限后停止并报告阻塞。

### 5. 重算 misc 页面项目亲和度与晋升候选 (`update_affinity`)
...
cross-link-apply-report.md + misc pages + page-registry.json

#### 产出

更新的 misc affinity + affinity-report.md + cross-link-report.md

#### 验收

1. skip 模式保持空报告且 vault 无变化；执行模式对每个 changed misc page 重算 incoming/outgoing 项目计数，确认 affinity 与 score>=3 候选准确

2. 检查只改 misc affinity 且报告 link/relation/page/orphan counts 可由实际 vault 重算

#### 流程控制

- 验收通过：转到 `record_and_refresh`。

- 验收失败：返回 `update_affinity`。

- 最多连续失败 `3` 次；达到上限后停止并报告阻塞。

### 6. 幂等记录 Cross-Link 并刷新可选 QMD (`record_and_refresh`)
...
cross-link/affinity reports + log/hot/QMD config

#### 产出

幂等 CROSS_LINK tracking + cross-link-completion.md + 可选 QMD refresh

#### 验收

1. skip 模式确认 log/hot/QMD/vault 零变化且 reason 可证；执行模式从最终 vault 重算 counts，确认 CROSS_LINK run_id 恰好一次、hot last-3/timestamp 与报告一致

2. 验证 QMD guard/update/embed/verify 状态准确，失败未回滚 Markdown；snapshot SHA 如报告则真实存在

3. 最终抽查 links、typed relationships、orphans 与 misc candidates 可发现且无 archive/readout/staging 越界写入
