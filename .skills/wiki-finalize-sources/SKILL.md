...
steps:
  - id: verify_completion_gate
    ...
    check_voting:
      ...
      - check: 解析 finalization plan JSON，逐项核对 Markdown 的 source、eligibility、counts、paths、hash、warnings 与 next action 是同一 canonical result 的准确投影
  ...
  - id: update_special_files
    ...
    check_voting:
      ...
      - check: 解析 special-files report JSON，逐项核对 Markdown 的 sources、events、counts、validation、warnings 与 write order 是同一 canonical result 的准确投影
  ...
  - id: commit_manifest
    ...
    check_voting:
      ...
      - check: 核对 QMD guard/update/embed/verification、最终报告和磁盘事实；解析 finalization report JSON，逐项核对 Markdown 的 source 状态、pages、manifest locator、verify、QMD、warnings 与 next action 是同一 canonical result 的准确投影；重试不会重复 index/log/hot/manifest 内容
    on_pass: done
