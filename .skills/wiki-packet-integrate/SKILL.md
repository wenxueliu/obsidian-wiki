...
此 skill 是 `workflows/wiki-packet-integrate.yaml` 的 Agent Skill 格式投影；workflow 是行为事实来源，本文件把其字段渲染为可直接执行的 Markdown 指令。
...
进入本 skill 时先绑定本次 invocation 的 `run_id` 与绝对 `artifacts_dir`。若父 workflow 已传入这两个值，原样继承；否则运行 `obsidian-wiki artifacts-create --workflow wiki-packet-integrate`，解析其 JSON 输出后绑定。
...
### 1. 校验父 Job 冻结上下文、页面契约及 Packet/inline 绑定 (`validate_packet`)
...
1. 只使用父 Job 已冻结的 canonical vault、write mode、link format、owner/taxonomy、Writing Profile、Knowledge Profile 和 page contract；不得再次调用 wiki-context 或 wiki-page-contract，不得重新加载或覆盖这些规则。
...
5. 校验 context vault 包含 Job、context write mode 等于 Job write_mode、active Knowledge Profile/Layout status/hash 等于 page contract；任一冻结绑定不一致即 fail closed，返回 coordinator 重新规划。
...
父 coordinator 的 wiki-context.json + page-contract.json + job.json 路径 + Packet 路径或 inline source_id/unit_id

#### 产出

transport-validation.md；inline extraction 仅保留在 worker context；除规范允许的失败状态外不产生 wiki 写入
...
- 验收失败：返回 `validate_packet`。

- 最多连续失败 `1` 次；达到上限后停止并报告阻塞。

### 2. 定位规范页面并制定增量归并计划 (`plan_merge`)
...
2. 先应用 page contract 冻结的 Knowledge Profile：按 scope 复核 source/item 兼容性，按 extraction.retain/omit 保留领域知识并省略可再生成或越界内容；明显 mismatch 严格执行 ask/stage/reject，不得自动切换 Profile/Layout。
3. 先做 cheap index pass：仅扫描 index.md、页面 title/aliases/tags/summary，再只打开高相关候选页面正文。
4. 对 Packet 中每个 durable item 决定：归并到现有 canonical page、创建真正的新页面、作为 profile omit/noise 省略、或显式记录分歧。Packet/unit 边界绝不能成为页面边界。
5. 对 durable items 依次执行 canonical topic routing、跨 item synthesis 与缺失 cross-reference discovery；每个结论仍需 Packet locator 支持。现有 wiki 是串行增量 reducer，不另做整文档 reduction，也不读取原始 source 或相邻 unit。
6. 新页面先依据 Knowledge Profile knowledge_types 与 routing prompt 选择 declared page type 或明确兼容别名，再调用已注册的 `obsidian-wiki wiki-route-resolve` 生成目标；existing page 目标也必须落在 content_roots。为每个计划变更列出 profile verdict、page type、resolver target/evidence、create/update/omit/disagreement、精确 locator、需要的 wikilink/typed relationship、inferred/ambiguous 标记，以及 direct/staged 下的目标 artifact。
7. 在产出目录写 merge-plan.md。此步骤保持只读，不修改 live wiki、_staging、Job 或 manifest。

#### 输入

transport-validation.md + page-contract.json + 当前 Packet 或 inline in-memory extraction + vault 的 cheap index 元数据与少量候选页面

#### 产出

merge-plan.md（页面级路由、provenance、关系与写入模式计划）

#### 验收

独立读取 transport-validation.md、可用的 Packet 或 inline extraction、merge-plan.md 和计划引用的候选页面。确认 Knowledge Profile scope/extraction/knowledge_types 已执行且没有自动换域；每个 durable item 都有合理路由；没有一 transport/一 unit 一页面的机械拆分；现有知识优先归并且分歧未被静默覆盖；每项保留 path/hash/unit/line/byte locator；推断和歧义有标记；新链接目标存在或由同一事务创建；计划符合 direct/staged 模式。确认此步骤没有产生任何 wiki/Job/manifest 写入。
...
### 3. 按 direct 或 staged 策略写入并验证页面 (`write_pages`)
...
transport-validation.md + merge-plan.md + 当前 Packet 或 inline in-memory extraction + 相关 live/staged 页面

#### 产出

已验证的 live 页面或 staged artifacts + integration-write-report.md；Job unit 尚未推进

#### 验收

先以确定性页面/schema/path 校验确认真实文件与 direct/staged 边界，再做一次语义审查，确认归并符合 merge-plan、精确 provenance、冲突标记与幂等要求；不要用多个 voter 重复机械校验

#### 流程控制

- 验收通过：转到 `advance_unit`。

- 验收失败：返回 `write_pages`。

- 最多连续失败 `5` 次；达到上限后停止并报告阻塞。

### 4. 页面验证后原子推进当前 unit (`advance_unit`)
...
transport-validation.md + integration-write-report.md + 最新 job.json + 当前 write mode

#### 产出

原子更新的 job.json + unit-advance-report.md；尚未执行 source finalization

#### 验收

用 package 状态函数复核 unit 只在页面验证后推进、direct/staged 计数严格分离、job.json 原子更新且 special files/manifest 未变化；不要重复执行语义页面审查

#### 流程控制

- 验收通过：转到 `report_completion`。

- 验收失败：返回 `advance_unit`。

- 最多连续失败 `5` 次；达到上限后停止并报告阻塞。

### 5. 汇总单 Packet 页面归并与 unit 推进结果 (`report_completion`)
...
本次 Packet 的全部 reports + 最新 Job metadata
