...
进入本 skill 时先绑定本次 invocation 的 `run_id` 与绝对 `artifacts_dir`。若父 workflow 已传入这两个值，原样继承；否则运行 `obsidian-wiki artifacts-create --workflow tag-taxonomy`，解析其 JSON 输出后绑定。
...
- `optional_reads`: owner AGENTS,taxonomy,index,manifest,active knowledge pack
...
2. 只扫描 active Knowledge Pack routing.content_roots 下的 live `.md`，排除 routing.skip_dirs/system_dirs/system_paths；从 YAML frontmatter 提取 tags，不把正文 hashtag 当标签。
