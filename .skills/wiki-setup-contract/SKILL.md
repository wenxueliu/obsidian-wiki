...
steps:
  - id: build_core_contract
    ...
    check: 解析 core JSON/Markdown，核对必需 templates/Knowledge Packs inventory、Profile/Layout/Routing hashes、contract hash、config defaults 和 create-only/minimal-repair 策略；拒绝 symlink/path escape/reserved target，确认除 artifacts 外零写入
